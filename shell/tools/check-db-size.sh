...
if [[ "$(docker container inspect -f '{{.State.Status}}' ${CONTAINER_POSTGRES})" = "running" ]]; then
    ...
    ret=$(
        echo
        docker exec ${CONTAINER_POSTGRES} psql -h ${POSTGRES_IP} -p ${POSTGRES_USER} -d ${POSTGRES_DBNAME} -c 'SELECT pg_database.datname,pg_size_pretty(pg_database_size(pg_database.datname)) AS size FROM pg_database'
    )
    echo_log "$ret"
fi
